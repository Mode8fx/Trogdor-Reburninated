...
void loadGameState() {
	...
	if (saveBin) {
		...
		menu_difficulty.setOptionChoice(MENU_ARROW_SPEED_INDEX, gameState.settings.arrowSpeed);
		...
		menu_other.setOptionChoice(MENU_STARTING_LEVEL_INDEX, gameState.settings.startingLevel);
		menu_other.setOptionChoice(MENU_KNIGHT_BEHAVIOR_INDEX, gameState.settings.knightBehavior);
		menu_other.setOptionChoice(MENU_LEVEL_TRAN_INDEX, gameState.settings.levelTran);
		menu_cheats.setOptionChoice(MENU_INF_LIVES_INDEX, gameState.settings.infLives);
		menu_cheats.setOptionChoice(MENU_SPEEDY_MODE_INDEX, gameState.settings.speedyMode);
		menu_cheats.setOptionChoice(MENU_NOCLIP_INDEX, gameState.settings.noclip);
		menu_cheats.setOptionChoice(MENU_DEBUG_MODE_INDEX, gameState.settings.debugMode);
		MENU_INF_LIVES->setLocked(gameState.settings.locked_infLives);
		...
		MENU_SPEEDY_MODE->setLocked(gameState.settings.locked_speedyMode);
		MENU_NOCLIP->setLocked(gameState.settings.locked_noclip);
		MENU_DEBUG_MODE->setLocked(gameState.settings.locked_debugMode);
		SDL_RWclose(saveBin);
	}
	else {
